...
import type { AnySegment, IndoorGraph, LngLat } from "../types";
...
describe("RouterCoordinator with fixture graph", () => {
  it("stitches outdoor + indoor segments to an existing room", async () => {
    ...
    const hasLevel1 = segs.some(
      (s) => s.type === "indoor" && (s as any).level === "1",
    );
  ...
  it("throws a clear error for unknown room keys", async () => {
    ...
    await expect(rc.routeGpsToRoom([0, 0], "NOPE")).rejects.toThrow(
      /room not found/i,
    );
  });
});
...
test("GPS -> entrance -> R-D stitches outdoor + indoor", async () => {
  ...
  expect(
    segs.some((s) => s.type === "indoor" && (s as any).level === "1"),
  ).toBe(true);
});
